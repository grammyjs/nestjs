...
export function createComposerDecorator<TArg extends ComposerMetadata['arg']>(method: ComposerMetadata['method']) {
  return (arg: TArg): MethodDecorator => {
    return (_target: any, _key?: string | symbol, descriptor?: TypedPropertyDescriptor<any>) => {
      ...
      const prevValue = Reflect.getMetadata(COMPOSER_METADATA, descriptor.value) || []
