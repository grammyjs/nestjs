...
import { GrammyModuleAsyncOptions, GrammyModuleOptions, GrammyOptionsFactory } from './interfaces'
import { GRAMMY_BOT_NAME, GRAMMY_MODULE_OPTIONS } from './nestjs-grammy.constants'
import { ListenersExplorerService, MetadataAccessorService } from './services'
import { createBotFactory, getBotName } from './utils'

import debug from 'debug'
const log = debug('nestjs-grammy:core:module')
...
export class GrammyCoreModule implements OnApplicationShutdown {
  ...
  public static forRootAsync(options: GrammyModuleAsyncOptions): DynamicModule {
    ...
    const grammyBotProvider: Provider = {
      provide: grammyBotName,
      useFactory: async (options: GrammyModuleOptions) => await createBotFactory(options),
  ...
  private static createAsyncProviders(options: GrammyModuleAsyncOptions): Provider[] {
  ...
  private static createAsyncOptionsProvider(options: GrammyModuleAsyncOptions): Provider {
    ...
    const inject = [(options.useClass || options.useExisting) as Type<GrammyOptionsFactory>]
    return {
      provide: GRAMMY_MODULE_OPTIONS,
      useFactory: async (optionsFactory: GrammyOptionsFactory) => await optionsFactory.createGrammyOptions(),
      inject,
    }
  }
}
